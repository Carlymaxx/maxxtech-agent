...
import { Send, Terminal, Globe, HelpCircle, ChevronDown, Plus, X, Image, Sparkles } from "lucide-react";
...
const CHIPS = [
  ...
  { label: "Generate image", icon: Sparkles, action: "Generate an image of: " },
...
export function ChatInput({ onSend, disabled }: ChatInputProps) {
  ...
  const handleSubmit = () => {
    ...
    const text = content.trim() || (attachedImages.length > 0 ? "What's in this image?" : "");
    ...
    items.forEach((item) => {
      if (item.type.startsWith("image/")) {
        ...
        reader.onload = (ev) => {
          ...
          setAttachedImages((prev) => [
            ...prev,
            { dataUrl, mimeType: file.type, name: "pasted-image.png" },
          ]);
        };
        reader.readAsDataURL(file);
      }
        ...
        {CHIPS.map((chip) => {
          ...
          return (
            <button
              key={chip.label}
              onClick={() => {
                if (chip.label === "Generate image") setModelId("gemini-2.5-flash-image");
        ...
        <Textarea
          ...
          placeholder={
            isImageGen
              ...
              : "Ask MaxxTech Agent anything... (paste images too)"
          }
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild disabled={disabled}>
                <button className="flex items-center gap-1.5 h-8 px-3 rounded-lg bg-secondary/60 hover:bg-secondary border border-border text-xs font-mono font-medium transition-colors max-w-[200px] truncate">
